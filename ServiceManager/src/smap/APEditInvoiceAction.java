...
public class APEditInvoiceAction extends HttpServlet{
	...
	public void doPost(HttpServletRequest request,
    	...
    	if (sCommandValue.compareToIgnoreCase(APEditInvoiceEdit.COMMAND_VALUE_FINDVENDOR) == 0){
			...
			String sRedirectString =
				...
				+ "*" + APEditInvoiceEdit.RETURNING_FROM_FIND_VENDOR_FLAG + "=TRUE"
				+ "*" + APEditInvoiceEdit.RECORDWASCHANGED_FLAG + "=" 
	    			+ clsManageRequestParameters.get_Request_Parameter(APEditInvoiceEdit.RECORDWASCHANGED_FLAG, request)
    	   	    + "*" + APEditInvoiceEdit.PARAM_TOGGLEUNAPPLIEDTABLE + "=" 
    	    			+ clsManageRequestParameters.get_Request_Parameter(APEditInvoiceEdit.PARAM_TOGGLEUNAPPLIEDTABLE, request)
			...
			try {
				...
				String sWarning = "after Find Vendor command - error redirecting with string: " + sRedirectString;
				smaction.redirectAction(
						sWarning, 
						"", 
						SMTableapbatchentries.lid + "=" + entry.getslid()
    	...
    	if (sCommandValue.compareToIgnoreCase(APEditInvoiceEdit.COMMAND_VALUE_UPDATEVENDORINFO) == 0){
    		smaction.getCurrentSession().setAttribute(APBatchEntry.OBJECT_NAME, entry);
			smaction.redirectAction(
				"", 
				"", 
				SMUtilities.SMCP_REQUEST_PARAM_DATABASE_ID + "=" + smaction.getsDBID()
					+ "&" + APEditInvoiceEdit.RETURNING_FROM_UPDATEVENDORINFO_FLAG + "=true"
    	    		+ "&" + APEditInvoiceEdit.RECORDWASCHANGED_FLAG + "=" 
	    			+ clsManageRequestParameters.get_Request_Parameter(APEditInvoiceEdit.RECORDWASCHANGED_FLAG, request)
    	   	    	+ "&" + APEditInvoiceEdit.PARAM_TOGGLEUNAPPLIEDTABLE + "=" 
    	    			+ clsManageRequestParameters.get_Request_Parameter(APEditInvoiceEdit.PARAM_TOGGLEUNAPPLIEDTABLE, request)
		...
		//If this class has been called from the 'calculate terms' button:
    	if (sCommandValue.compareToIgnoreCase(APEditInvoiceEdit.COMMAND_VALUE_CALCULATETERMS) == 0){
    		smaction.getCurrentSession().setAttribute(APBatchEntry.OBJECT_NAME, entry);
			smaction.redirectAction(
				"", 
				"", 
				SMUtilities.SMCP_REQUEST_PARAM_DATABASE_ID + "=" + smaction.getsDBID()
					+ "&" + APEditInvoiceEdit.RETURNING_FROM_CALCULATETERMS_FLAG + "=true"
    	    		+ "&" + APEditInvoiceEdit.RECORDWASCHANGED_FLAG + "=" 
	    			+ clsManageRequestParameters.get_Request_Parameter(APEditInvoiceEdit.RECORDWASCHANGED_FLAG, request)
    	   	    	+ "&" + APEditInvoiceEdit.PARAM_TOGGLEUNAPPLIEDTABLE + "=" 
    	    			+ clsManageRequestParameters.get_Request_Parameter(APEditInvoiceEdit.PARAM_TOGGLEUNAPPLIEDTABLE, request)
    	...
    	//Process apply line
    	if (sCommandValue.compareToIgnoreCase(APEditInvoiceEdit.COMMAND_VALUE_APPLYTODOC) == 0){
    		String sApplyToDocNumber = clsManageRequestParameters.get_Request_Parameter(APEditInvoiceEdit.APPLYTODOCNUMBER_TO_APPLY_PARAM, request);
    		try {
	    		entry.applyLineToDocNumber(sApplyToDocNumber, getServletContext(), smaction.getsDBID(), smaction.getUserName(), smaction.getFullUserName());
	    		APBatch batch = new APBatch(entry.getsbatchnumber());
	    		batch.updateBatchEntry(entry, getServletContext(), smaction.getsDBID(), smaction.getUserID(), smaction.getFullUserName());
			} catch (Exception e) {
				smaction.getCurrentSession().setAttribute(APBatchEntry.OBJECT_NAME, entry);
				smaction.redirectAction(
					"Could not apply to document number '" + sApplyToDocNumber + "' - " + e.getMessage(), 
					"", 
    	    		SMTableapbatches.lbatchnumber + "=" + sBatchNumber
    	    		+ "&" + SMTableapbatchentries.lentrynumber + "=" + sEntryNumber
    	    		+ "&" + SMTableapbatchentries.lid + "=" + entry.getslid()
    	    		+ "&" + APEditInvoiceEdit.RECORDWASCHANGED_FLAG + "=" 
	    			+ clsManageRequestParameters.get_Request_Parameter(APEditInvoiceEdit.RECORDWASCHANGED_FLAG, request)
	    			+ "&" + APEditInvoiceEdit.RETURN_TO_TABLES_BOOKMARK + "=Y"
	    			+ "&" + APEditInvoiceEdit.PARAM_TOGGLEUNAPPLIEDTABLE + "=" 
    	    			+ clsManageRequestParameters.get_Request_Parameter(APEditInvoiceEdit.PARAM_TOGGLEUNAPPLIEDTABLE, request)
    	    		+ "&" + "CallingClass=" + sCallingClass
				);
			...
			if (smaction.getOriginalURL().trim().compareToIgnoreCase("") != 0){
				...
				smaction.redirectAction(
					"", 
					"Line was successfully applied to document number '" + sApplyToDocNumber + "'.",
	   	    		SMTableapbatches.lbatchnumber + "=" + entry.getsbatchnumber()
    	    		+ "&" + SMTableapbatchentries.lentrynumber + "=" + entry.getsentrynumber()
    	    		+ "&" + SMTableapbatchentries.lid + "=" + entry.getslid()
    	    		+ "&" + SMTableapbatchentries.ientrytype + "=" + entry.getsentrytype()
    	...
    	//Process remove line
    	if (sCommandValue.compareToIgnoreCase(APEditInvoiceEdit.COMMAND_VALUE_REMOVELINE) == 0){
    		String sLineNumber = clsManageRequestParameters.get_Request_Parameter(APEditInvoiceEdit.LINE_NUMBER_TO_DELETE_PARAM, request);
    		try {
