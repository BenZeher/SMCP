...
public class AREditCustomerGroups extends HttpServlet {

	private static final long serialVersionUID = 1L;
	private static final String sObjectName = "Customer Group";
	private static final String sCalledClassName = "AREditCustomerGroupsEdit";
	public void doPost(HttpServletRequest request,
		...
		try{
	        ...
	        ResultSet rs = clsDatabaseFunctions.openResultSet(
	     	...
	     	out.println ("<SELECT NAME=\"" + ARCustomerGroup.ParamsGroupCode + "\">" );
        	
        	while (rs.next()){
        		sOutPut = "<OPTION VALUE=\"" + rs.getString(SMTablearcustomergroups.sGroupCode).trim() + "\">";
        		sOutPut += rs.getString(SMTablearcustomergroups.sGroupCode).trim() + " - " + rs.getString(SMTablearcustomergroups.sDescription).trim();
	        	out.println (sOutPut);
        	}
        	rs.close();
	    	...
	    	System.out.println("[1579116402] Error in " + this.toString() + " class!!");
		...
		sOutPut = "<P><INPUT TYPE=SUBMIT NAME='SubmitEdit' VALUE='Edit Selected " + sObjectName + "' STYLE='width: 2.50in; height: 0.24in'></P>";
		sOutPut = sOutPut + "<P><INPUT TYPE=SUBMIT NAME='SubmitDelete' VALUE='Delete Selected " + sObjectName + "' STYLE='width: 2.50in; height: 0.24in'>";
		sOutPut = sOutPut + "  Check to confirm deletion: <INPUT TYPE=CHECKBOX NAME=\"ConfirmDelete\">";
		sOutPut = sOutPut + "<P><INPUT TYPE=SUBMIT NAME='SubmitAdd' VALUE='Add New " + sObjectName + "' STYLE='width: 2.50in; height: 0.24in'></P>";
