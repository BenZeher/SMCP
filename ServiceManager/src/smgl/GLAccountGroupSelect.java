...
public class GLAccountGroupSelect extends HttpServlet {

	private static final long serialVersionUID = 1L;

	private static final String sCalledClassName = "GLAccountGroupEdit";
	public static final String OBJECT_NAME = GLAccountGroup.ParamObjectName;
	...
	public void doPost(HttpServletRequest request,
	    ...
	    out.println("<A HREF=\"" + WebContextParameters.getdocumentationpageURL(getServletContext()) + "#" + Long.toString(SMSystemFunctions.GLEditAccountGroups) 
	    		+ "\">Summary</A><BR><BR>");
		...
		try{
	        String sSQL = "SELECT * FROM " + SMTableglaccountgroups.TableName 
	        	+ " ORDER BY " + SMTableglaccountgroups.sdescription;
	        ResultSet rs = clsDatabaseFunctions.openResultSet(
	     	...
	     	out.println ("<SELECT NAME=\"" + SMTableglaccountgroups.lid + "\">" );
	     	String sSelected = "";
        	while (rs.next()){
        		String sLid = Long.toString(rs.getLong(SMTableglaccountgroups.lid));
        		if (entry.getlid().compareToIgnoreCase(sLid) == 0){
        			sSelected = "selected";
        		}else{
        			sSelected = "";
        		}
        		out.println ("<OPTION " + sSelected + " VALUE=\"" + Long.toString(rs.getLong(SMTableglaccountgroups.lid)) + "\">");
        		out.println (rs.getString(SMTableglaccountgroups.sgroupcode) + " - " + rs.getString(SMTableglaccountgroups.sdescription));
        	}
        	rs.close();
			...
			out.println("<BR><FONT COLOR=RED><B>Error [1528404772] reading " + OBJECT_NAME + "s  - " + ex.getMessage() + ".</B></FONT><BR>");
			//return false;
		}
