...
public class GLAccountSegmentSelect extends HttpServlet {

	private static final long serialVersionUID = 1L;

	private static String sCalledClassName = "GLAccountSegmentEdit";
	public static final String OBJECT_NAME = GLAccountSegment.ParamObjectName;
	...
	public void doPost(HttpServletRequest request,
	    ...
	    out.println("<A HREF=\"" + WebContextParameters.getdocumentationpageURL(getServletContext()) + "#" + Long.toString(SMSystemFunctions.GLEditAccountSegments) 
	    		+ "\">Summary</A><BR><BR>");
		...
		try{
	        String sSQL = "SELECT * FROM " + SMTableglaccountsegments.TableName 
	        ...
	        ResultSet rs = clsDatabaseFunctions.openResultSet(
	     	...
	     	out.println ("<SELECT NAME=\"" + SMTableglaccountsegments.lid + "\">" );
	     	String sSelected = "";
        	while (rs.next()){
        		String sLid = Long.toString(rs.getLong(SMTableglaccountsegments.lid));
        		if (entry.getlid().compareToIgnoreCase(sLid) == 0){
        			sSelected = "selected";
        		}else{
        			sSelected = "";
        		}
        		out.println ("<OPTION " + sSelected + " VALUE=\"" + Long.toString(rs.getLong(SMTableglaccountsegments.lid)) + "\">");
        		out.println (rs.getString(SMTableglaccountsegments.sdescription));
        	}
        	rs.close();
			...
			out.println("<BR><FONT COLOR=RED><B>Error [1523053038] reading " + OBJECT_NAME + "s  - " + ex.getMessage() + ".</B></FONT><BR>");
			//return false;
		}
