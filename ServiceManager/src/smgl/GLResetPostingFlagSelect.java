...
public class GLResetPostingFlagSelect extends HttpServlet {
	...
	public void doGet(HttpServletRequest request,
	    ...
	    String title = "GL Reset Posting-In-Process Flag";
	    String subtitle = "";
	    ...
	    out.println("<A HREF=\"" + WebContextParameters.getdocumentationpageURL(getServletContext()) 
	    	+ "#" + Long.toString(SMSystemFunctions.GLResetPostingInProcessFlag) 
	    	+ "\">Summary</A><BR>");
	    
	    out.println("<BR>This function will reset the system's 'Posting-In-Process' flag so that batches"
	    		+ " can be posted and regular GL processes can continue.  The flag is set when a posting"
	    ...
	    try{
	    	...
	    	if (rs.next()){
	    		if (rs.getInt(SMTablegloptions.ibatchpostinginprocess) == 0){
	    			out.println("NOTE: Currently, the GL posting-in-process flag is already cleared, it does not need to be reset.<BR>");
	    		}else{
	    			out.println("A previous posting is not completed - "
        					+ SMUtilities.getFullNamebyUserID(rs.getString(SMTablegloptions.luserid), getServletContext(), sDBID, ".GLResetPostingFlagSelect") + " has been "
        					+ rs.getString(SMTablegloptions.sprocess) + " "
        					+ "since " + rs.getString(SMTablegloptions.datstartdate) + ".<BR><BR>");
    	...
    	out.println ("<FORM ACTION =\"" + SMUtilities.getURLLinkBase(getServletContext()) + "smgl.GLResetPostingFlagAction\">");
