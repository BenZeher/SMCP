...
public class ICEditReceiptEntry extends HttpServlet {
	...
	private static final String sReceiptEntryObjectName = "Receipt Entry";
	...
	public void doPost(HttpServletRequest request,
	    ...
	    String subtitle = "";
	    if (m_bIsNewEntry){
	    	title = "Edit NEW " + sReceiptEntryObjectName;
	    }else{
	    	title = "Edit " + sReceiptEntryObjectName + ": " + m_Entry.sEntryNumber();	
	    }
		...
		if (!createFormFromEntryInput(m_Entry, m_bEditable, sDBID, sUserID,sUserFullName,m_pwOut)){
			response.sendRedirect(
					"" + SMUtilities.getURLLinkBase(getServletContext()) + "smic.ICEditBatchesEdit"
					+ "?BatchNumber=" + m_Entry.sBatchNumber()
					+ "&BatchType=" + m_sBatchType
	...
	private void displayEntryHeaderFields (PrintWriter m_pwOut, ICEntry m_Entry, boolean m_bEditable){
	    ...
	    if (m_bEditable){
	    	m_pwOut.println("<BR><INPUT TYPE=SUBMIT NAME='SubmitEdit' VALUE='Save " + sReceiptEntryObjectName + "' STYLE='height: 0.24in'>");
	    	m_pwOut.println("  <INPUT TYPE=SUBMIT NAME='Delete' VALUE='Delete " + sReceiptEntryObjectName + "' STYLE='height: 0.24in'>");
	...
	private void Display_Line_Header(PrintWriter m_pwOut){
		...
		m_pwOut.println("<TD CLASS=\"" + SMMasterStyleSheetDefinitions.TABLE_CELL_HEADING_LEFT_JUSTIFIED + "\">Receipt number</TD>");
