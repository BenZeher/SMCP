package ServletUtilities;

import java.awt.BasicStroke;
import java.awt.Color;
import java.awt.Graphics2D;
import java.awt.image.BufferedImage;
import java.io.ByteArrayInputStream;
import java.io.ByteArrayOutputStream;
import java.io.File;
import java.io.IOException;


import javax.imageio.ImageIO;

import org.apache.commons.codec.binary.Base64;

public class clsBase64Functions {

	public static String getbase64EncodedStringFromJSON (
		String jsonString,
		int iImageWidth,
		int iImageHeight,
		int iStrokeWidth,
		int iRColor,
		int iGColor,
		int iBColor
		)throws Exception{
		
		BufferedImage img = convertJsonToImage(
			jsonString,
			iImageWidth,
			iImageHeight,
			iStrokeWidth,
			iRColor,
			iGColor,
			iBColor
		);
		
		return encodeToBase64String(img, "png");
	}

	public static void writeJSONToPNGFile(
			String jsonString,
			int iImageWidth,
			int iImageHeight,
			int iStrokeWidth,
			int iRColor,
			int iGColor,
			int iBColor,
			String sFullFileName
			) throws Exception{
		
		writePNGImageFile(sFullFileName, convertJsonToImage(
			jsonString, 
			iImageWidth, 
			iImageHeight, 
			iStrokeWidth, 
			iRColor, 
			iGColor, 
			iBColor
		));
	}

	public static void writePNGImageFile(String sFullFileName, BufferedImage img) throws Exception{
		try {
			File outputfile = new File(sFullFileName);
			ImageIO.write(img, "png", outputfile);
		} catch (Exception e) {
			throw new Exception("[1394822805] error writing image file to disk - " + e.getMessage());
		}
	}

	public static BufferedImage convertJsonToImage(
			String jsonString,
			int iImageWidth,
			int iImageHeight,
			int iStrokeWidth,
			int iRColor,
			int iGColor,
			int iBColor
			) throws Exception{
		
		//TJR - 10/23/2015- this was added because we suddenly started getting errors on our hosted CentoOS 6.7 server like this:
		/*
		java.lang.NoClassDefFoundError: Could not initialize class java.awt.image.BufferedImage
			ServletUtilities.base64Functions.convertJsonToImage(base64Functions.java:80)
			ServletUtilities.base64Functions.getbase64EncodedStringFromJSON(base64Functions.java:27)
			
		Apparently something changed on the server because the program still ran ok in all the other environments.
		But after some research, and a few failed attempts at setting a headless variable in the tomcat start options, etc.,
		I added the following lines and ran it as an 'smtest' app.  It worked the first time AND after that, the old
		version of the program ALSO ran without error.  So I am assuming these lines set the environment for the whole
		tomcat server and so the old program worked as well.  Still not sure, but I plan to leave it in for now.
		 */
		//if(!GraphicsEnvironment.getLocalGraphicsEnvironment().isHeadless()){
		System.setProperty("java.awt.headless", "true");
			//System.out.println("[1445616677] - set to headless");
		//}
		
	    BufferedImage offscreenImage = new BufferedImage(iImageWidth, iImageHeight, BufferedImage.TYPE_INT_RGB);
	    Graphics2D g2 = offscreenImage.createGraphics();
	    g2.setColor(Color.white);
	    g2.fillRect(0,0,iImageWidth,iImageHeight);
	    
	    //First draw the signature line:
		//g2.setPaint(new Color(iRLineColor, iGLineColor, iBLineColor));
		//g2.setStroke(new BasicStroke(iLineStrokeWidth));
		//g2.drawLine(iLineMargin, iLineTop, iImageWidth - iLineMargin, iLineTop);
	
		//Now prepare to draw the signature:
	    g2.setStroke(new BasicStroke(iStrokeWidth));
	    g2.setPaint(new Color(iRColor, iGColor, iBColor));
	    int iCurrentPosition = 0;
	    while (iCurrentPosition < jsonString.length()){
	    	int ilx = 0;
			int ily = 0;
			int imx = 0;
			int imy = 0;
			try {
				iCurrentPosition = jsonString.indexOf("lx", iCurrentPosition);
				if (iCurrentPosition == -1) {break;}
				ilx = Integer.parseInt(jsonString.substring(iCurrentPosition + "lx:".length() + 1, jsonString.indexOf(",", iCurrentPosition)));
				iCurrentPosition = jsonString.indexOf("ly", iCurrentPosition);
				ily = Integer.parseInt(jsonString.substring(iCurrentPosition + "ly:".length() + 1, jsonString.indexOf(",", iCurrentPosition)));
				iCurrentPosition = jsonString.indexOf("mx", iCurrentPosition);
				imx = Integer.parseInt(jsonString.substring(iCurrentPosition + "mx:".length() + 1, jsonString.indexOf(",", iCurrentPosition)));
				iCurrentPosition = jsonString.indexOf("my", iCurrentPosition);
				imy = Integer.parseInt(jsonString.substring(iCurrentPosition + "my:".length() + 1, jsonString.indexOf("}", iCurrentPosition)));
			} catch (Exception e1) {
				throw new Exception("Error [1394822799] drawing graphics lineparsing JSON string - at position " + iCurrentPosition 
					+ " - "+ e1.getMessage() + ".");
			}
	    	//Draw the line:
	    	try {
				g2.drawLine(ilx, ily, imx, imy);
			} catch (Exception e) {
				throw new Exception("[1394822800] error drawing graphics line - " 
					+ Integer.toString(ilx) + ", "
					+ Integer.toString(ily) + ", "
					+ Integer.toString(imx) + ", "
					+ Integer.toString(imy) + " - "
					+ e.getMessage() + ".");
			}
	    }
	
	    return offscreenImage;
	}

    public static String encodeToBase64String(BufferedImage image, String type) throws Exception{
    	// 'type' is image file type, like "png", "jpeg", "bmp", etc.
    	
    	//See note above from 10/23/2015
    	System.setProperty("java.awt.headless", "true");
    	    	
        String imageString = null;
        ByteArrayOutputStream bos = new ByteArrayOutputStream();
        
        try {
            ImageIO.write(image, type, bos);
            byte[] imageBytes = bos.toByteArray();

         // TJR - 12/26/2018 - modified this to use Apache commons library instead of sun library:
            
            Base64 encoder = new Base64();
            imageString = encoder.encodeAsString(imageBytes);

            bos.close();
        } catch (IOException e) {
            throw new Exception("Error [1434743356] converting image to Base64 - " + e.getMessage());
        }
 
        /* ORIGINAL CODE - using the sun base64 encoder:
        String imageString = null;
        ByteArrayOutputStream bos = new ByteArrayOutputStream();

        try {
            ImageIO.write(image, type, bos);
            byte[] imageBytes = bos.toByteArray();

            BASE64Encoder encoder = new BASE64Encoder();
            imageString = encoder.encode(imageBytes);

            bos.close();
        } catch (IOException e) {
            throw new Exception("Error [1434743356] converting image to Base64 - " + e.getMessage());
        }
        return imageString;
         */
        
        return imageString;
    }
    public static BufferedImage decodeBase64ToImage(String imageString) {
    	//See note above from 10/23/2015
    	System.setProperty("java.awt.headless", "true");
    	
        BufferedImage image = null;
        byte[] imageByte;
        try {
        	
        	// TJR - 12/26/2018 - modified this to use Apache commons library instead of sun library:
        	
            //BASE64Decoder decoder = new BASE64Decoder();
            //imageByte = decoder.decodeBuffer(imageString);
            
            //Base64 decoder = new Base64();
            imageByte = Base64.decodeBase64(imageString);

        	ByteArrayInputStream bis = new ByteArrayInputStream(imageByte);
            image = ImageIO.read(bis);
            bis.close();
        	
        	
        } catch (Exception e) {
            e.printStackTrace();
        }
        return image;
    }
    
	public static String loadFileIntoBase64Image(String sFullImageFilePath, String sImageType) throws Exception{
		String s = "";
		//s = "\"";
		//BufferedImage bufferedImage = ImageIO.read(Files.newInputStream(Paths.get(basePath + imageSource)));
		if (sImageType == null){
			//Try to pick off the file type from the suffix:
			sImageType = clsServletUtilities.getFileSuffix(sFullImageFilePath);
		}
		
		//Do a little filtering:
		if (sImageType.compareToIgnoreCase("jpeg") == 0){
			sImageType = "jpg";
		}
		BufferedImage bufferedImage = null;
		try {
			//bufferedImage = ImageIO.read(Files.newInputStream(Paths.get(sFullImageFilePath)));
			bufferedImage = ImageIO.read(new File("webapps" + sFullImageFilePath));
			
		} catch (Exception e) {
			throw new Exception("Error [1487687331] converting image file '" + sFullImageFilePath + "' into buffered image - " + e.getMessage());
		}
		s = clsBase64Functions.encodeToBase64String(bufferedImage, sImageType);
		return s;
	}
}
