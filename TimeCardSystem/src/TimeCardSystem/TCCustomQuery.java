...
public class TCCustomQuery extends java.lang.Object{
	...
	public void processReport(
		...
		if(sQueryID.compareToIgnoreCase("") != 0){
			TCLogEntry log = new TCLogEntry(conn);
			log.writeEntry(sUserName, TCLogEntry.LOG_OPERATION_QUERY, "SMQUERY" + " Saved Query: " + sQueryID, sQueryTitle + " - SQL: '" + sQueryString + "'", "[1376509318]");
		}
		else{
			TCLogEntry log = new TCLogEntry(conn);
		    log.writeEntry(sUserName, TCLogEntry.LOG_OPERATION_QUERY, "SMQUERY", sQueryTitle + " - SQL: '" + sQueryString + "'", "[1376509318]");
		
		};

		sQueryString = sQueryString.replaceAll("\\n", "");
		sQueryString = sQueryString.replaceAll("\\r", "");
		sQueryString = sQueryString.replace(SESSIONTAG_VARIABLE, sSessionTag);
		sQueryString = sQueryString.replace(USER_VARIABLE, sUserName);
		sQueryString = sQueryString.replace(LINKBASE_VARIABLE, clsServletUtilities.getURLLinkBase(context));
			...
			if (iEnd > -1){
				...
				if (isSetVariableParameter(sParam)){
					...
					try {
						...
						String sSetCommand = sParam.replace(TCCustomQuery.SETVARIABLECOMMAND, "");
						sSetCommand = sSetCommand.replace(TCCustomQuery.STARTINGPARAMDELIMITER, "");
						sSetCommand = sSetCommand.replace(TCCustomQuery.ENDINGPARAMDELIMITER, "");
					...
					s = s.replace(TCCustomQuery.STARTINGPARAMDELIMITER + sParam + TCCustomQuery.ENDINGPARAMDELIMITER, "");
				}
			}
		}
